 ...
 */ 
typedef // - > notas/Ex/@P_01

 struct s_int_Array 
 { 
   int   length; 
   ints  data  ; // - > ints e um apontador de inteiro ( int * insts )
   int   ix    ; 
 } int_Array; 
...
typedef int_Array* ref_int_Array; // - > ref_int_Array recebe os parametros que contem em int_Array ( length, data, ix ) 
// end P_01
...
{ 
 ...
 // reciclar espaco 
   ...
   /**executar o metodo auxiliar  
    //printIntMatrix ( matrix );  
    ^
    |_ achei interessante colocar esta parte, apos reciclar tudo, e aterrar meus apontadores e printar. pode se ver como entra em loop, afinal
    a condicao de parada ja nao existe mais, ele tenta printar e so encontra lixo e invasao de espaço! uma pratica horrivel.
 */
 // encerrar 
    IO_pause ( "Apertar ENTER para continuar" ); 
} // end method_06 ( )
