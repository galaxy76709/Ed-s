...
//---------------------
   // Método setLength (realoca memória)
    void setLength(int newLength) {
        // liberar memória anterior se houver
        if (data != nullptr) {
            delete[] data;
        }
...
// Verifica se o arquivo terminou corretamente (sem lixo extra)
    if (!file.eof()) {
        ...
        file >> resto;
        if (!resto.empty()) {
            cout << "Aviso: ha dados extras no arquivo que foram ignorados." << endl;
        }
    }
...
void sortDown() {
    if (data != nullptr && length > 1) {
        ...
        std::cout << "Erro: arranjo vazio ou com tamanho invalido." << std::endl;
    }
}
//---------------------
...
//---------------------
void recordArrayToFile (std::string nomeArquivo, Array<int> arr) {

    std::ofstream arquivo(nomeArquivo);

    arquivo << arr.length << std::endl;

    for (int i = 0; i < arr.length; i = i + 1) {

        arquivo << arr.data[i] << std::endl;
    }
...
//---------------------
void  readArrayFromFile_median ( string fileName ) { 
   //vai procurar o arquivo com o nome 
   ifstream file ( fileName ); 
